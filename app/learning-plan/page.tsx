...
export default function LearningPlanPage() {
  ...
  const [planDays, setPlanDays] = useState<StudyDayPlan[]>([]);
  ...
  const [workflow2Result, setWorkflow2Result] = useState("");
  const [sourceNotice, setSourceNotice] = useState("请先从 JD 分析页完成差距项分析，Flow3 会使用那次 Workflow2 输出生成计划。");
  const autoGeneratedRef = useRef(false);

  const canGenerate =
    Boolean(today) && isValidStudyDays(studyDays) && isValidStudyHours(hoursPerDay) && workflow2Result.trim().length > 0;
  const calendarCells = useMemo(() => (calendarMonth ? buildCalendarCells(calendarMonth) : []), [calendarMonth]);
  ...
  useEffect(() => {
    const clientToday = getStartOfToday();
    setToday(clientToday);
    setCalendarMonth(new Date(clientToday.getFullYear(), clientToday.getMonth(), 1));
    setPlanDays(mapResponseToPlanDays(initialResponse.dailyPlans, clientToday));
    setSelectedDateKey(formatDateKey(clientToday));
  }, []);
  ...
  useEffect(() => {
    ...
    const storedInput = readStoredStudyPlanInput();
    if (!storedInput) {
      return;
    }

    setWorkflow2Result(storedInput.workflow2_result);
    setSourceNotice(buildSourceNotice(storedInput));

    if (!autoGeneratedRef.current) {
      autoGeneratedRef.current = true;
      void generatePlan(storedInput.workflow2_result, defaultStudyDays, defaultHoursPerDay, today);
    }
  }, [today]);

  async function handleGeneratePlan() {
    if (!canGenerate) {
      setError(workflow2Result.trim() ? "请输入 1-14 天的学习天数，以及有效的每日学习时间。" : "请先在 JD 分析页完成差距项分析。");
      return;
    }
    ...
    await generatePlan(workflow2Result, studyDays, hoursPerDay, today);
  }

  async function generatePlan(
    workflow2ResultInput: string,
    nextStudyDays: number,
    ...
    try {
      const response = await fetchJson<StudyPlanResponse>("/api/study-plan", {
        method: "POST",
        body: JSON.stringify({
          workflow2_result: workflow2ResultInput,
          study_days: nextStudyDays,
          daily_hours: nextHoursPerDay
        })
      });

      const nextPlanDays = mapResponseToPlanDays(response.dailyPlans, baseDate);
      setStudyPlanResponse(response);
      setPlanDays(nextPlanDays);
      setSelectedDateKey(nextPlanDays[0]?.dateKey ?? formatDateKey(baseDate));
    ...
    return (
      <div className="min-h-screen bg-[#f7f7f4] text-ink">
        ...
        <main className="pt-[72px]">
          <div className="mx-auto max-w-[1280px] px-6 py-10 text-sm text-muted lg:px-8">正在加载学习计划...</div>
        </main>
      </div>
    );
  }
          ...
          <section className="rounded-[24px] border border-line bg-white p-5 shadow-card">
            ...
            <div className="mt-3 grid gap-3 lg:grid-cols-[200px_236px_160px]">
              ...
              <InputField
                label="每天投入时间"
                value={hoursPerDay}
                onChange={(value) => setHoursPerDay(clampInteger(Number(value), 1, 24))}
                min={1}
                max={24}
                step={1}
                suffix="小时 / 天"
                      ...
                      <div className="mt-3 max-h-[180px] space-y-1.5 overflow-y-auto pr-1">
                        {dayPlan.tasks.map((task) => {
                          ...
                          return (
                            <div
                              key={task.id}
                              className="group flex items-start gap-2 rounded-xl bg-white pl-1.5 pr-2 py-2 transition hover:bg-[#faf6e8]"
                            >
                              <button
                                type="button"
                                onClick={() => toggleTask(dayPlan.id, task.id)}
                                className={`mt-0.5 grid h-4 w-4 flex-none place-items-center rounded-md border text-[10px] transition ${
                                  task.completed
                              ...
                              {editing ? (
                                <input
                                  ...
                                  placeholder="新建一条具体任务，不超过42字"
                                  className="w-full flex-1 bg-transparent text-xs leading-5 outline-none placeholder:text-[#b1b1b1]"
                                />
                              ) : (
                                <button
                                  type="button"
                              ...
                              <button
                                ...
                                className="ml-1 mt-0.5 text-sm leading-none text-[#d84a4a] opacity-0 transition group-hover:opacity-100"
                                aria-label="删除任务"
                              >
                                ×
                              </button>
...
function readStoredStudyPlanInput(): StoredStudyPlanInput | null {
  try {
    const rawValue = window.sessionStorage.getItem(studyPlanInputStorageKey);
    if (!rawValue) {
      return null;
    }

    const parsed = JSON.parse(rawValue) as Partial<StoredStudyPlanInput>;
...
function buildSourceNotice(input: StoredStudyPlanInput) {
  const tags = [input.job_type, input.company_type].map((item) => item?.trim()).filter(Boolean);
  const suffix = tags.length > 0 ? `（${tags.join(" / ")}）` : "";
  return `已读取上一页 Workflow2 差距项分析${suffix}，Flow3 将基于这份差距项生成学习计划。`;
}
...
function mapResponseToPlanDays(dailyPlans: DailyPlanItem[], baseDate: Date) {
...
function formatHours(value: number) {
  return `${Math.max(1, Math.round(value))} 小时`;
}
